...
export function WebGLHero({ backgroundOnly = false }: { backgroundOnly?: boolean } = {}) {
  ...
  const initGL = () => {
    ...
    const resizeCanvas = () => {
      ...
      const dpr = backgroundOnly ? 0.5 : Math.min(1.0, (typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1))
  ...
  if (backgroundOnly) {
    return (
      <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden z-0">
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full opacity-50" style={{ background: "transparent" }} />
      </div>
    )
  }
