...
vec3 StarLayer(vec2 uv, float starSpeed, float hueShift, float saturation, float twinkleIntensity, float time, float speed, float glowIntensity) {
  vec3 col = vec3(0.0);
...
void main() {
  ...
  for (float i = 0.0; i < 1.0; i += 1.0 / NUM_LAYER) {
    ...
    col += StarLayer(uv * scale + i * 453.32, uStarSpeed, uHueShift, uSaturation, uTwinkleIntensity, uTime, uSpeed, uGlowIntensity) * fade;
  }
...
export default function Galaxy({
  ...
  const focalSerialized = JSON.stringify(focal);
  const rotationSerialized = JSON.stringify(rotation);
  ...
  useEffect(() => {
    ...
    const focalArr = JSON.parse(focalSerialized);
    const rotationArr = JSON.parse(rotationSerialized);
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uFocal: { value: new Float32Array(focalArr) },
        uRotation: { value: new Float32Array(rotationArr) },
