...
export default function App() {
  ...
  if (showVideoStream) {
    return (
      <div className="h-screen w-full bg-[#030612]">
  ...
  if (showFileConverter) {
    return (
      <div className="h-screen w-full bg-[#030612] p-4 sm:p-6">
  ...
  return (
    <div className="min-h-screen bg-[#030612] text-white font-sans selection:bg-purple-500/30 flex flex-col relative overflow-hidden">
      {/* Background orbs to match Chitti Robo AI */}
      <div className="absolute top-1/4 left-1/4 w-[350px] h-[350px] bg-purple-600/10 rounded-full blur-[100px] pointer-events-none animate-orb-slow-1" />
      <div className="absolute bottom-1/3 right-1/4 w-[400px] h-[400px] bg-cyan-600/10 rounded-full blur-[120px] pointer-events-none animate-orb-slow-2" />

      {/* Header */}
      <header className="border-b border-purple-500/15 bg-[#070b19]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 cursor-pointer border border-transparent hover:border-purple-500/20 px-2 py-1 rounded-xl transition-all" onClick={() => { setShowAssistantPage(false); setShowVideoEditor(false); setShowVoiceCommand(false); setShowVideoStream(false); setShowFileConverter(false); setShowPdfConverter(false); setMode('ai-assistant'); window.history.pushState({}, '', '/'); }}>
              <div className="w-8 h-8 bg-gradient-to-br from-purple-600 via-indigo-600 to-cyan-500 rounded-lg flex items-center justify-center shadow-lg shadow-purple-500/20">
                <Sparkles className="w-5 h-5 text-white animate-pulse" />
              </div>
              <span className="text-xl font-bold tracking-tight text-white font-display">OmniGen AI</span>
            </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 border rounded-xl transition-all text-xs font-bold ${showSidebar
                ...
                : 'bg-white/5 border-purple-500/15 text-white/80 hover:text-white'
                }`}
          ...
          <nav className="flex items-center gap-6 text-sm font-medium text-white/60">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 mt-4 w-48 bg-[#070b19]/90 backdrop-blur-xl border border-purple-500/15 rounded-2xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col p-2 gap-1 z-50">
                <button
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className={`border-r border-purple-500/15 bg-[#070b19]/80 backdrop-blur-xl transition-all duration-300 flex flex-col overflow-hidden ${showSidebar ? 'w-72 opacity-100 border-r border-purple-500/15' : 'w-0 opacity-0 border-r-transparent border-r-0'
          }`}>
